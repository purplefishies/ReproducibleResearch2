...
tornadoes <- ddat %>%
  ...
  group_by( EVTYPE ) %>% summarise(ave_fatal=mean(FATALITIES,rm.na=TRUE))
#   summarise( ave_fatal = mean(FATALITIES,rm.na=TRUE),ave_injuries=mean(INJURIES,rm.na=TRUE),ave_propdmg = mean(PROPDMG,rm.na=TRUE),ave_cropdmg = mean(CROPDMG,rm.na=TRUE))
```
#     
