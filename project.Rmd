...
An example is the following. To perform the filtering for "wind", I would start with the following filter
```{r,echo=TRUE}
dat %>% filter( grepl(".*wind.*", EVTYPE,perl=T,ignore.case=T) ) %>%
    select(EVTYPE) %>% unique()
```
Then I would inspect the results, and discover that several other FACTORS had been accounted for. These included "flood","cold","hurricane". Hence these FACTORS would need to be removed in order to establish what effect this one condition of "wind" had on the statistics I was interested in studying.
